...
class _FirstTimeUserScreenState extends State<FirstTimeUserScreen> {
  ...
  Future<void> _promptWifiCredentials() async {
    ...
    try {
      ...
      if (result != null && result['connected'] == true) {
        ...
        if (serialNumber != null) {
          ...
          // TR: WiFi bilgilerini sor ve SSH ile gönder (iPhone'da servis alınamadığı için) | EN: Ask for WiFi credentials and send via SSH (because service can't be accessed on iPhone) | RU: Спросить учетные данные WiFi и отправить через SSH (так как сервис недоступен на iPhone)
          await _promptWifiCredentials();
